...
public interface IGen24Service
{
    ...
    ValueTask<T?> SendFroniusCommand<T>(string request, JsonNode? jToken = null, CancellationToken token = default) where T : Gen24NoResultCommand, new();
    ...
    ValueTask<(string JsonString, HttpStatusCode StatusCode)> GetFroniusStringResponse(string request, JsonNode? jToken = null, IEnumerable<HttpStatusCode>? allowedStatusCodes = null, CancellationToken token = default);
    ValueTask<(JsonNode Token, HttpStatusCode StatusCode)> GetFroniusJsonResponse(string request, JsonNode? jToken = null, IEnumerable<HttpStatusCode>? allowedStatusCodes = null, CancellationToken token = default);
    ...
    Task<Gen24SystemPreservation?> GetSystemPreservation(CancellationToken token = default);
