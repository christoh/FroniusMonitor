...
| `InverterDetailsViewTests` | The inverter page with real gauges: the ΔFrequency gauge and the two Δ voltage groups gone while the inverter is not synchronized, the user's switch still having the last word, and the cos(phi) group's `DialShowsAbsoluteValue` and scale - see [[InverterDetailsView.Lifecycle]] |
